'''
The following code has been modified by the paper authors based on the original code.
The original code is available at: https://github.com/PaddlePaddle/PaddleSeg/blob/develop/EISeg/tool/eiseg2paddlex.py
...
# if you encounter error in Qt, please install the following package
conda install qtpy
# or
pip install pyqt5
'''
    ...
    for name in tqdm(imgs_name):
        ...
        if ext.lower() in FORMATS:
            ...
            save_lab_path = osp.join(
                save_lab_folder, os.path.split(lab_path)[-1])
...
parser.add_argument('--save_path', '-d',
                    help='Path to save folder, Required', required=True)
parser.add_argument('--image_path', '-o',
                    help='Path of image folder, Required', required=True)
parser.add_argument('--label_path', '-l',
                    help='Path of label folder', default=None)
parser.add_argument(
    '--mode', '-m', help='Mode of dataset, seg/det', default="seg")
parser.add_argument('--split_rate', '-s',
                    help='Proportion of training data and evaluation data', default=0.9)
